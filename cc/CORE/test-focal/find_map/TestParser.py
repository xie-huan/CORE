...
class TestParser():
	...
	def get_function_metadata(class_identifier, function_node, blob: str):
		...
		identifier_node_list = TestParser.children_of_type(function_node, "identifier")
		for m in identifier_node_list:
			identifier = TestParser.match_from_span(m, blob)
			if 'test' in identifier:
				metadata['testcase'] = True
