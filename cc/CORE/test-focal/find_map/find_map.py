...
def analyze_project(repo_id, grammar_file, tmp, output):
    """
    ...
    # subprocess.call(['git', 'clone', repo_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def find_map_test_cases(root, grammar_file, language, output, repo):
    ...
    focal_path=os.path.join(output, "focal.txt")
    f1=open(focal_path, "w")
    ...
    # Test Classes
    try:
        # if "Chart" in str(root):
        # result = subprocess.check_output(['find', '-name', "*Tests.java"])  #Chart
        result = subprocess.check_output(['find', '-name', "*Test.java", "-o", "-name", "Test*.java"])  # except Chart&Time
        # result = subprocess.check_output(['find', '-name', "Test*.java"])  # Time
        # result = subprocess.check_output(r'grep -l -r @Test --include \*.java', shell=True)
        tests = result.decode('ascii').splitlines()
        tests = [j.replace("./", "") for j in tests]
        # print(tests)
        """elif "Closure" in str(root):
        result = subprocess.check_output(['find', '-name', "*Test.java"])
        # result = subprocess.check_output(r'grep -l -r @Test --include \*.java', shell=True)
        tests = result.decode('ascii').splitlines()
        tests = [j.replace("./", "") for j in tests]
        # print(tests)
    ...
    focals = [f for f in focals if not "tests" in f]
    # focals_norm = [f.lower().replace("source","") for f in focals ] # Chart source(.java) & tests(Tests.java)
    # focals_norm = [f.lower().replace("src", "") for f in focals]  # src(.java) & test(Test.java)
    # focals_norm = [f.lower().replace("java", "", 1) for f in focals]  # src/java(.java) & src/test(Test.java)
    focals_norm = [f.lower().replace("main", "") for f in focals]  # src/main/java(.java) & src/test/java(Test.java)
    for f in focals_norm:
        f1.write(f+'\n')
        #print(focals_norm)
    ...
    for test in tests:
        # tests_norm = test.lower().replace("tests", "").replace("junit/","") # source(.java) & tests(Tests.java)
        tests_norm = test.lower().replace("test", "")
        t1.write(tests_norm + '\n')
        # print(tests_norm)
        # tests_norm = test.lower().replace("/src/test/", "/src/main/")
        # tests_norm = tests_norm.replace("Tests", "").replace("junit", "")
...
def main():
    ...
    analyze_project(repo_id, grammar_file, tmp, output)
